create or alter procedure SP_SYSO_ESTOQUE_VINC_ITEM_MOV (
    ID_EMPRESA integer,
    ID_FILIAL integer,
...
BEGIN
    ...
    SELECT COUNT(*) FROM MOVIMENTO_ITENS_USUARIOS MIU
    WHERE (ID_EMPRESA = :ID_EMPRESA)
        AND (ID_FILIAL = :ID_FILIAL)
        AND (ID_MOVIMENTO = :ID_MOVIMENTO)
        AND (ID_PRODUTO = :ID_PRODUTO)
    ...
    SELECT QUANT_INTEIRA FROM MOVIMENTO_ITENS MI
    WHERE (ID_EMPRESA = :ID_EMPRESA)
        AND (ID_FILIAL = :ID_FILIAL)
        AND (ID_MOVIMENTO = :ID_MOVIMENTO)
        AND (ID_PRODUTO = :ID_PRODUTO)
    INTO :QUANT_INTEIRA_MOVIMENTO;
    ...
    BEGIN
     ...
     IF(:ICOUNT > 0) THEN
     BEGIN
        IF (:QUANT_INTEIRA = :QUANT_INTEIRA_MOVIMENTO) THEN
            BEGIN
                IF (:ICOUNT_MIU = 0 ) THEN
                BEGIN
                    INSERT INTO MOVIMENTO_ITENS_USUARIOS(
                        ID_EMPRESA,
                        ID_FILIAL,
                ...
                UPDATE MOVIMENTO_OS
                SET
                    ID_ADICIONAL_OS = :IDENTIFICADOR_OS
                WHERE
                    ID_MOVIMENTO = :ID_MOVIMENTO AND
